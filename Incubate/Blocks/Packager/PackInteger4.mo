...
model PackInteger4 "Pack integer value into CAN message"
  ...
  Interfaces.PackWriteOut packWriteOut annotation (Placement(transformation(
          extent={{86,-22},{126,18}}), iconTransformation(
        extent={{-20,-20},{20,20}},
        rotation=90,
        origin={108,0})));
equation
  when packWriteOut.trigger then
    CANMessage.integerBitPacking(packWriteOut.msg, bitStartPosition, length, u);
  end when;
