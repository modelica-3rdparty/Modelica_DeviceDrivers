...
class MQTT "A driver for Message Queue Telemetry Transport."
  ...
  encapsulated function constructor "Creates an MQTT instance."
	...
	input Integer retryInterval = 20 "Maximum time (in seconds) for retries";
    ...
    output MQTT mqtt;
    external "C" mqtt = MDD_mqttConstructor(provider, address, port, receiver, QoS, channel, bufferSize, clientID, userName, password, trustStore, keyStore, privateKey, keepAliveInterval, cleanSession, reliable, connectTimeout, retryInterval, MQTTVersion, disconnectTimeout, enableServerCertAuth, verify, sslVersion)
      annotation (
