...
model TestSerialPackager_SharedMemoryNoCleanup
  "Example for combining SharedMemoryNoCleanup and SerialPackager blocks"
extends Modelica.Icons.Example;
  ...
  Communication.SharedMemoryWriteNoCleanup
                                  sharedMemoryWrite annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={-30,-68})));
  ...
  Communication.SharedMemoryReadNoCleanup
                                 sharedMemoryRead annotation (Placement(
        transformation(
        extent={{-10,-10},{10,10}},
        rotation=270,
        origin={50,50})));
...
<b>Note:</b> There is no causality between the <code>sharedMemoryWrite</code> block and the <code>sharedMemoryRead</code> block. Therefore the execution order of the blocks is not determined. This indeterminism may also show up in the plots.
    ...
    Icon(graphics={Bitmap(extent={{-100,-100},{100,100}}, fileName=
              "modelica://Modelica_DeviceDrivers/Resources/Images/Icons/DooFi-Trash-Bins.png")}));
end TestSerialPackager_SharedMemoryNoCleanup;
