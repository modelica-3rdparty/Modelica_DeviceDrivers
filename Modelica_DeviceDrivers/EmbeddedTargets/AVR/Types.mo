within Modelica_DeviceDrivers.EmbeddedTargets.AVR;
package Types
...
type Platform = enumeration(
      ...
      ATmega16
           "AVR ATmega16",
      ATmega328P
             "AVR ATmega328P (e.g. Arduino Uno)")
...
type TimerSelect = enumeration(
      Timer0
         "Usually an 8-bit timer",
      Timer1
         "Usually a 16-bit timer",
      Timer2
         "Usually an 8-bit timer")
...
type TimerNumber = enumeration(
      A
    "Use this when the timer only has 1 clock associated with it",
      B)
...
type VRefSelect = enumeration(
      AREF
       "AREF used as VRef and internal VRef is turned off",
      AVCC
       "AVCC with external capacitor at the AREF pin is used as VRef (5V on Arduino)",
      Internal2
            "This is a reserved setting in most AVRs. On ATmega1280, it is 1.1 internal reference voltage",
      Internal
           "Internal reference voltage (1.1V (ATmega168/328) or 2.56V on (ATmega8, ATmega1280))")
