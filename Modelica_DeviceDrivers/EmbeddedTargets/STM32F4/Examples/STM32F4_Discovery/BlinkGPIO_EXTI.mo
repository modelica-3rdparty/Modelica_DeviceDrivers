...
model BlinkGPIO_EXTI
  ...
  Functions.Digital.InitLed p2 = Functions.Digital.InitLed(handle  = HALinit, led = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.LED.LED4);
  Functions.Digital.InitLed p3 = Functions.Digital.InitLed(handle  = HALinit, led = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.LED.LED5);
  Functions.Digital.InitLed p4 = Functions.Digital.InitLed(handle  = HALinit, led = Modelica_DeviceDrivers.EmbeddedTargets.STM32F4.Types.LED.LED6);
...
<h1>BlinkGPIO_EXTI</h1>
<p>BlinkGPIO_EXTI is a model configuring intterrupt on falling edge on GPIO Line 0. Since modelica is not able to generate code for the <a href=modelica://Modelica_DeviceDrivers/Resources/Scripts/OpenModelica/EmbeddedTargets/STM32F4/Examples/STM32F4_Discovery/ext_callback.c>callback function</a>, the user has to edit code here manually. In the callback function all User LEDs (port D pin 12 - 15) are toggled. As a result, the LEDs are toggled when the blue user button is released. Use this model to see if your Modelica tool can export code for STM32F4 MCUs.</p>
