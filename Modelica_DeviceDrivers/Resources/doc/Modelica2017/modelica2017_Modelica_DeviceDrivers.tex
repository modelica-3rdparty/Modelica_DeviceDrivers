...
However, the \emph{ExternalDevices} library was limited to Microsoft Windows and
was developed to be used only with the Dymola tool. In the further course of
development it was decided to split the \emph{ExternalDevices} library into the
commercial DLR Visualization library and an open-source cross-platform hardware
interface library, the \emph{Modelica\_DeviceDrivers} library. The library is
available from its GitHub project site at
\url{https://github.com/modelica/Modelica_DeviceDrivers/}. This paper is based
on a pre-release of MDD v1.5.0. We plan to officially release v1.5.0 shortly
...
Marshalling (LCM) datagrams\footnote{LCM project site,
...
MDD v1.5 introduces the new top-level package \modelica{EmbeddedTargets}. The
...
limitations.
Currently, there exists first prototypical support for the
Atmel\footnote{Atmel, \url{http://www.atmel.com}} AVR family of
microcontrollers. This is ongoing work and the interface is not fixed, yet. A
prototype application is described in Section~\ref{sec:EmbeddedControl}.
...
With the help of the \emph{Modelica\_DeviceDrivers} serial port implementation, the Arduino can be utilized to make sensor data available in a real-time Modelica model, as depicted in Figure \ref{fig:arduino} \footnote{Autodesk screen shots reprinted courtesy of Autodesk, Inc.}.
...
hardware, no threading support, limited memory allocation, etc., the
general-purpose external functions in MDD cannot be used.

MDD.EmbeddedTargets contains blocks and functions to directly control
I/O or clocks in the AVR ATmega microcontroller family\footnote{At this moment
only ATmega16 and ATmega328P (=Arduino Uno).
The code can easily be extended, but requires checking the data sheets in order to write to the correct bits.}.
The reason for adding this code to the MDD library is that it makes it
simple to write a model for a microcontroller that works the same way
in any Modelica tool that supports generation of minimal C-code.

The Modelica code itself tries to avoid the Integer constants from the
datasheets. Instead, enumerations such as prescaler=1/128 or clock=2B are
passed from Modelica and the C-code for the AVR target depends on
function inlining in order to remove dead code.
For example, the constructor for the clock takes an enumeraion for which
clock to manipulate and after function inlining, the C-code for other
clocks is removed.
The blocks in the MDD library try to take user-friendly
constants\footnote{Parameters with Evaluate=true are avoided since the
C-code depends on function inlining, which means constants should be passed.},
such as frequency=100Hz or period=0.1 for real-time synchronization; the
Modelica code then has logic to find good clock prescalers to create a
matching frequency.

The EmbeddedTargets models do not support simulating the microcontroller
