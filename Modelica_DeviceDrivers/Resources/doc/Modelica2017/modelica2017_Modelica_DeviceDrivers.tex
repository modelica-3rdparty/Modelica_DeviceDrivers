...
function constructor "Initialize timer"
  ...
  output Timer timer;
  external "C" ... annotation (Include="#include \"MDDAVRTimer.h\"");
end constructor;
\end{lstlisting}
\begin{lstlisting}[language=C]
...
\begin{figure}
  ...
  \caption{The MDD model of the single board heater system exposes uses the pulse-width modulation (PWM) block to control the heater and fan, and uses the analog-to-digital converter (ADC) block to read the temperature. Note that the SBHS board uses hardware components to create a linear mapping from voltage to temperature where 0V maps to 0℃.}
  \label{fig:sbhsboard}
\end{figure}
\begin{figure}
  \centering
  \includegraphics[width=0.5\columnwidth]{figures/SBHSBoardWithController.pdf}
  \caption{The MDD example model of the single board heater system running at 125 Hz together with a real-time PID controller trying to control the fan such that the temperature is at the setpoint of 45℃ while the heat source is fed by a constant voltage.}
