...
\textsf{Function Layer}. Modelica's external objects allow external functions to access the internal memory (created by the \modelica{constructor}) between calls to external functions, \textit{i.e.}, the so
created device handlers are maintained in memory for accessing them in subsequent simulation phases.
...
Having such a standardized directory structure facilitated creating cross-platform libraries with external C library
dependencies. For example, the Modelica code snippet below declares a
dependency to the header file \modelica{MDDKeyboard.h} and the libraries
...
allows the propagation of a triggering event in the inverse connector direction.
Its supporting logic is omitted here for brevity. Variable
\modelica{userPkgBitSize} allows the propagation of a user defined package size which
