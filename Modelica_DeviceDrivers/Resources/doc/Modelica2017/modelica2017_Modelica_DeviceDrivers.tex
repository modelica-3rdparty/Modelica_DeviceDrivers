...
interface library, the \emph{Modelica\_DeviceDrivers} library.
Since 20xx the library is developed by the Modelica Association at GitHub\footnote{Modelica\_DeviceDrivers project,
\url{https://github.com/modelica/Modelica_DeviceDrivers}}.
% This paper is based on a pre-release of MDD v1.5.0. We plan to officially release v1.5.0 shortly before the conference.

\TBEU{Bernhard: State that MDD is developed by the Modelica Association since 20xx.}
\TBEU{Bernhard: This pre-relase note should not go into the paper. I commented it out.}
...
As of MDD v1.5.0 it contains only one subpackage which
...
\TBEU{Update text and reference to 3.3 revision 1?, MLS 3.2 and 3.3 are already mentioned above in the text (lines 161 and 164), thus the citations should go there.}
...
\TBEU{If it is ongoing work, should it be better moved to the Incubate package until it's ready to be released?}
...
The \modelica{EmbeddedTargets} package contains blocks and functions to directly
control I/O or clocks in the AVR ATmega microcontroller family\footnote{As of MDD
...
in any Modelica tool, if the Modelica tool supports the generation
of suitably efficient, minimal C code.

The Modelica code itself tries to avoid the Integer constants from the
data sheets. Instead, enumerations such as \modelica{prescaler}=1/128 or \modelica{clock}=2B are
...
matching frequency.
\TBEU{Martin: TBH, I do not get the idea behind constants over evaluated parameters. Your aim is to generate minimal C code, right? So, \emph{minimal} in the sense of memory usage of the application, I guess. Can you better explain by means of a Modelica and C code example code snippets what you are trying to achieve and how did you do it? And it is tool-specific for OpenModelica only?}
...
\TBEU{Is this really realistic? I mean we also have no ambition to emulate the CAN bus ona desktop.}
