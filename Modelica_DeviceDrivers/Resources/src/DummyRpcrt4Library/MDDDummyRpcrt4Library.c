 ...
 *
 * - <B>The problem</B>: We need to specify in Modelica to link the Rpcrt4.lib library if running on Windows, but
 *   it is not needed for Linux (and results in a linking error if specified in the
 *   annotation).
 * - <B>The workaround</B>: We provide a dummy libRpcrt4.a library for Linux, so that linking won't result in an
