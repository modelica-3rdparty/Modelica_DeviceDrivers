 ...
 *
 * - <B>The problem</B>: We need to specify in Modelica to link the libcrypto library if running on Linux, but
 *   it is not needed for Windows (and results in a linking error if specified in the
 *   annotation).
 * - <B>The workaround</B>: We provide a dummy crypto.lib library for Windows, so that linking won't result in an
