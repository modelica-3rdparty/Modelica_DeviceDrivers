...
The problem: We need to specify in Modelica to link the libssl library if running on Linux, but
             it is not needed for Windows (and results in a linking error if specified in the
             annotation).

The workaround: We provide a dummy ssl.lib library for Windows, so that linking won't result in an
