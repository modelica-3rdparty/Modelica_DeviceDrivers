...
<p>Looking at the examples in<i> .Blocks.Examples</i> (or <i>.ClockedBlocks.Examples</i>, respectively) the usage of the library should be self-explanatory. In the following two exemplarily examples are considered.</p>
<p><h5>User Input devices</h5></p>
<p> E.g., using a joystick or gamepad as input for a real-time simulation just requires to drag &AMP; drop two blocks: <i>.Blocks.OperatingSystem.SynchronizeRealtime</i> and <i>.Blocks.InputDevices.JoystickInput</i> (or respectively, <i>.ClockedBlocks.OperatingSystem.SynchronizeRealtime</i> and <i>.ClockedBlocks.InputDevices.JoystickInput</i>). See the executable example at <i>.(Clocked)Blocks.Examples.TestInputJoystick</i>:</p>
<p><img src=\"modelica://Modelica_DeviceDrivers/Resources/Images/TestInputJoystick.png\"/></p>
<p>The <i>SynchronizeRealtime</i> block synchronizes the simulation time with the operating systems real-time clock. That allows interactive Modelica simulations, e.g., a vehicle driving simulation using a gamepad for user inputs.</p>
